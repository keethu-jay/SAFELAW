...
TEST_XML_DIR = BACKEND_DIR / "Test Files" / "xml docs"
FINAL_DATASET_DIR = BACKEND_DIR.parent / "Final Dataset"
RAW_XML_DIR = BACKEND_DIR.parent / "data" / "raw_xml"
...
def classify_paragraph(paragraph: str, model_client) -> str:
...
def _core_text_for_dedup(text: str) -> str:
    """Strip refs and normalize so near-identical passages (e.g. punctuation variants) dedupe together."""
    t = (text or "").strip()
    # Remove leading [N] or [sN] citation patterns repeatedly
    while True:
        ...
        t = t[m.end() :].strip()
    t = t.lower()[:900]
    # Normalize punctuation and whitespace so "First, one" = "First one", "latter—in" = "latter, in"
    t = re.sub(r"[\"\'”“]|''", " ", t)
    t = re.sub(r"[\—\-\–,;:]\s*", " ", t)
    # Join hyphenated line-breaks: "neighbour-\nhood" -> "neighbourhood"
    t = re.sub(r"-\s+", "", t)
...
def is_same_case(source_file: str, doc_id: str) -> bool:
    """Return True if doc_id refers to the same case as source_file (test input)."""
    ...
    if src == hit:
        return True
    # Match when core case name (without citation) is same, e.g. "McLoughlin v O'Brian" vs "McLoughlin v O'Brian [1982] UKHL 3"
    ...
    if src_core and hit_core:
        # Same if one contains the other (allows "mcloughlin v o'brian" to match "mcloughlin v o'brian [1982] ukhl 3")
        if src_core in hit_core or hit_core in src_core:
            return True
        # Or if first 20 chars match (catch variants)
...
def run_for_corpus(corpus_key: str, config: Dict[str, str], row_indices: List[int] = None, update_tsv: bool = False, use_classification: bool = True) -> Path:
    ...
    def get_top_10(test_paragraph: str, source_file: str, retries: int = 5, rpc_override: str = None):
        ...
        for attempt in range(retries):
            try:
                # Fetch more candidates so we can filter same-case and still get 10 unique
                max_res = 300 if "sentences" in rpc else 500
                r = supabase.rpc(
                    rpc,
                    {
                ...
                # Filter: exclude same-case (test file), skip duplicate/near-duplicate texts
                filtered = []
                seen_texts = set()
                seen_substrings = set()  # 100-char substrings to catch embedded duplicates
                SUBSTR_LEN, SUBSTR_STEP = 100, 30
                for hit in hits:
                    doc_id = (hit.get("doc_id") or "").strip()
                    text = (hit.get("text") or "").strip()
                    
                    # Skip suggestions from the same file as the test text
                    if source_file and doc_id and is_same_case(source_file, doc_id):
                        continue
                    
                    # Skip duplicate content: same passage with different [N] refs or punctuation counts as duplicate
                    core = _core_text_for_dedup(text)
                    if not core:
                        continue
                    if core in seen_texts:
                        continue
                    # Skip if very similar prefix (catches "First one has..." with minor tail differences)
                    min_overlap = 80
                    if any(
                        (n := min(130, len(core), len(prev))) >= min_overlap and core[:n] == prev[:n]
                        for prev in seen_texts
                    ):
                        continue
                    # Skip if any substantial substring (100 chars) was already seen (catches same passage embedded in different paragraphs)
                    core_substrs = {core[i:i + SUBSTR_LEN] for i in range(0, max(0, len(core) - SUBSTR_LEN) + 1, SUBSTR_STEP) if len(core[i:i + SUBSTR_LEN]) >= 80}
    ...
    if update_tsv and row_indices is not None:
        output_path = BACKEND_DIR / "testing_scripts" / "output" / f"Classification_Comparison_{corpus_key}.tsv"
    ...
    for idx, (court_opinion, file_name, test_para) in enumerate(rows):
        ...
        if use_classification and openai_client:
            cls = classify_paragraph(test_para, openai_client)
        ...
        # Fallback: use cosine RPC when paragraphs KNN times out (cosine can be faster)
        if len(top10) == 0 and rpc_name == "match_corpus_mini_paragraphs_knn":
            top10 = get_top_10(text_to_embed, file_name, rpc_override="match_corpus_mini_paragraphs")
            if top10:
                print(f"  Row {idx+1} using fallback (cosine) - KNN timed out")
        if len(top10) == 0:
