#!/usr/bin/env python3
# v4: label-filtered + context tag. Combines v1 (filter by role) + v2 (context metadata).
# Uses corpus_documents_mini_sentences_context_tag with filter_by_label=True.
# Out: testing_scripts/output/Classification_Comparison_sentences_v4_label_filtered_context_tag.tsv
...
def get_text_v4(idx, court_opinion, file_name, test_para, test_label, case_metadata, get_case_summary):
...
def main():
    ...
    print("=== v4: label-filtered + context tag ===")
    run(
        ...
        out_suffix="v4_label_filtered_context_tag",
        get_text_to_embed=get_text_v4,
        supabase=supabase,
        model=model,
